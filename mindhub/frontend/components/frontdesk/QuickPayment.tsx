...
export default function QuickPayment({ onPaymentComplete }: QuickPaymentProps) {
  ...
  const loadPendingPayments = async (patientId: string) => {
    try {
      const response = await fetch(`http://localhost:8080/api/v1/frontdesk/payments/pending/${patientId}`);
      const data = await response.json();
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/v1/frontdesk/payments/process', {
        method: 'POST',
        headers: {
  ...
  const payPendingAmount = async (pendingPayment: PendingPayment) => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/v1/frontdesk/payments/pay-pending/${pendingPayment.id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
