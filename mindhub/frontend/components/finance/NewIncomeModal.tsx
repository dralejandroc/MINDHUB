...
export default function NewIncomeModal({ selectedDate, onClose, onSave }: NewIncomeModalProps) {
  ...
  const handlePatientSearch = async (searchTerm: string) => {
    ...
    try {
      ...
      const response = await authGet(`/api/expedix/django/patients?search=${encodeURIComponent(searchTerm)}`);
      console.log('RESPONSE PATIENT MODAL',response);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await authPost('/api/finance/django', payload);
  ...
  // get patients when modal opens
  useEffect(() => {
    const fetchInitialPatients = async () => {
      try {
        console.log('llamando');
        setSearchLoading(true);
        const response = await authGet('/api/expedix/django/patients/');
